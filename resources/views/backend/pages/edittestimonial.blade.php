...
<div class="p-6 sm:p-8 w-full max-w-4xl mx-auto font-sans text-slate-800 relative">
    ...
    <div class="mb-8">
        <a href="{{ route('testimonials') }}" class="inline-flex items-center gap-2 text-slate-500 hover:text-brand-600 transition-colors mb-4">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            Back to Testimonials
        </a>
        <h2 class="text-3xl font-extrabold text-slate-900 tracking-tight">Edit Testimonial</h2>
        <p class="text-sm text-slate-500 mt-1">Update feedback from {{ $testimonial->name }}</p>
    </div>

    <div class="bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
        <form action="{{ route('testimonial.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
            @csrf
            
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                
                <div class="md:col-span-1">
                    <label class="block text-sm font-bold text-slate-700 mb-3">Student Photo</label>
                    <div class="relative group aspect-square">
                        <div class="w-full h-full rounded-2xl overflow-hidden border-2 border-slate-100 shadow-inner bg-slate-50">
                            <img id="previewImage" 
                                src="{{ $testimonial->image ? asset('uploads/testimonials/'.$testimonial->image) : 'https://ui-avatars.com/api/?name='.$testimonial->name }}" 
                ...
                <div class="md:col-span-2 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-bold text-slate-700 mb-2">Rank</label>
                                <input type="text" name="rank" value="{{ $testimonial->rank }}"
                                    class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-all">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-slate-700 mb-2">Year</label>
                                <input type="text" name="year" value="{{ $testimonial->year }}"
                                    class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-all">
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="quote" rows="5" required
                            class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-all resize-none">{{ $testimonial->quote }}</textarea>
                    </div>
                </div>
            </div>

            <div class="pt-8 border-t border-slate-100 flex items-center justify-end gap-4">
                <a href="{{ route('testimonials') }}" class="px-8 py-3 rounded-xl font-bold text-slate-500 hover:bg-slate-50 transition-all">
                    Discard Changes
                </a>
